...
let evaluation_error msg => raise (Vm.Evaluation_error msg);

let type_error msg arg => evaluation_error (msg ^ ": " ^ Vm.Value.to_string arg);
...
let builtin_arithmetic op zero one cat state args => {
  let nums = extract_numbers op args;
  let result =
    switch nums {
    | [] =>
      switch zero {
...
let builtin_concat state args => {
  let strs = extract_strings "concat" args;
  push state (Sexp.Str (String.concat "" strs))
};

let builtin_length state =>
  fun
...
let builtin_compare op num_compare str_compare state args => {
  ...
  | [x, ...xs] =>
    Sexp.(
      switch x {
      | Num x =>
        let xs = extract_numbers op xs;
        push state (Sexp.Bool (cmp num_compare x xs))
      | Str x =>
        let xs = extract_strings op xs;
...
let builtin_callcc state =>
  fun
  | [f] => {
      let cont = capture_cont state;
      apply state f [cont]
    }
  | _ => evaluation_error "Builtin function call/cc takes one argument";

let builtin_eval state =>
  fun
  | [s] => {
      let result =
        switch (Vm.eval (Vm.context state) s) {
...
let builtin_macroexpand name recurse state =>
  fun
  | [s] => {
      let result =
        switch (Vm.macroexpand recurse (Vm.context state) s) {
        | Ok v => Sexp.Cons (Sexp.Bool true) v
        | Error e => Sexp.Cons (Sexp.Bool false) (Sexp.Str e)
        };
      push state result
...
let register context =>
  List.iter
    (fun (name, f) => Vm.register_builtin name f context)
    [
      ("cons", builtin_cons),
      ("exit", builtin_exit),
      ("error", builtin_error),
