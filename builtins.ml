...
let builtin_println state args =
  List.iter (function
      | Sexp.Str s -> print_string s
      | s -> type_error "Cannot print non-string argument" s
    ) args;
...
let builtin_macroexpand state = function
  | [s] -> push state (Vm.macroexpand true (Vm.context state) s)
  | _ -> evaluation_error ("Builtin function macroexpand takes 1 argument")

let builtin_macroexpand_once state = function
  | [s] -> push state (Vm.macroexpand false (Vm.context state) s)
  | _ -> evaluation_error ("Builtin function macroexpand-1 takes 1 argument")
...
let register context =
  List.iter (fun (name, f) ->
    ...
    "macroexpand-1", builtin_macroexpand_once;
  ]
